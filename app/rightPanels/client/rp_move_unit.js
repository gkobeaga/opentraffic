...
Template.rp_move_unit.helpers({
	castle_or_village: function() {
		...
		return (selected.type == 'castle' || selected.type == 'village');
	},
...
Template.rp_move_unit.events({
	...
	'click #move_unit_button': function(event, template) {
		...
		if (selected && selected.type == 'castle' || selected.type == 'village') {
			...
			Meteor.call('create_army_from_building', get_selected_units(), selected.type, selected.id, get_unit_moves())
			Session.set('rp_template', 'rp_info_'+selected.type)
		}
