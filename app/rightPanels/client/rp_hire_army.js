...
Template.rp_hire_army.events({
	...
	'click #hire_army_hire_button': function(event, template) {
		...
		if (no_units_are_selected()){
			...
			Meteor.call('hire_army', get_selected_hiring_units(), this._id, selected.type, function(error, result) {
				if (error) {
					$(alert).show(100);
...
Template.rp_hire_army.created = function() {
	...
	self.isHiringForVassal = false;

	this.autorun(function() {
		var data = Template.currentData();
		if (data) {
			self.isHiringForVassal = data.user_id != Meteor.userId();
		} else {
			self.isHiringForVassal = false;
		}
	})
