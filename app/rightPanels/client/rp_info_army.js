...
Template.rp_info_army.events({
	...
	'click #build_village_button': function(event, template) {
		...
		Meteor.call('build_village', self.x, self.y, function(error, result) {
			if (error) {
				$(alert).show()
				...
				Session.set('selected', {type:'village', id:result, x:self.x, y:self.y});
			}
		})
	},
...
Template.rp_info_army.created = function() {
	...
	self.autorun(function() {
		if (Template.currentData() && Number.isInteger(Template.currentData().x) && Number.isInteger(Template.currentData().y)) {
	...
	self.autorun(function() {
		...
		if (selected && Template.currentData() && selected.type == 'army') {
		...
		if (Template.currentData()) {
			...
			if (Session.get('mouse_mode') != 'finding_path') {
				...
				if (selected.type == 'army') {
					...
					draw_army_highlight(selected.id)
					var moves = Moves.find({army_id:selected.id})
