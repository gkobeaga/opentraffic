...
Cue.addJob('updateNetTotalEveryone', {retryOnError:false, maxMs:1000*60*10}, function(task, done) {
	updateNetTotalEveryone()
	done()
})

// runs at the end of resource collection
updateNetTotalEveryone = function() {
	var marketPrices = Market.find().fetch();
	...
	Meteor.users.find({}, {fields:fields}).forEach(function(user) {
		updateNetworth_total_quick(user, marketPrices);
	})
}
...
updateNetworth_total_quick = function(user, marketPrices) {
	if (!user.net.armies) {
	...
	_.each(marketPrices, function(res) {
		worth += res.price * resources[res.type];
	})

	check(worth, validNumber);
	Meteor.users.update(user._id, {$set:{"net.total":worth}});
